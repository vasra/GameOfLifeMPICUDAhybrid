...
CUDA_PATH ?= /apps/compilers/cuda/10.1.168
...
ifneq ($(TARGET_ARCH),$(HOST_ARCH))
    ...
    ifeq ($(TARGET_ARCH)-$(TARGET_OS),aarch64-qnx)
        ...
        NVCCFLAGS += -arbin $(QNX_HOST)/usr/bin/aarch64-unknown-nto-qnx7.1.0-ar
        ifneq ($(TARGET_FS),)
...
FLAGS = ${OPTFLAG} -I${CUDA_PATH}/include -Wall -g ${INCDIR}
NVCCFLAGS = -O3 -I${CUDA_PATH}/include --compiler-options -fno-strict-aliasing --ptxas-options=-v ${INCDIR} -std=c++11
BITS = $(shell getconf LONG_BIT)
ifeq (${BITS},64)
        LIBSUFFIX := 64
endif
LFLAGS = -L${CUDA_PATH}/lib${LIBSUFFIX} -lm -lstdc++ -lcudart
CLEAN_FILES = gol.a
